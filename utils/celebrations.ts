...
/**
 * Celebration manager to coordinate confetti, haptics, and sound
 */
export class CelebrationManager {
    private confettiRef: React.RefObject<ConfettiCannon> | null = null;
    ...
    setConfettiRef(ref: React.RefObject<ConfettiCannon>) {
     ...
     * Requirement 3.2, 7.4: Combined confetti, haptic feedback, and sound
     */
    async celebrateGoalCompletion() {
        ...
        // Get optimized confetti config
        const confettiConfig = getOptimizedConfettiConfig("goalCompletion");

        if (!confettiConfig) {
            // Confetti disabled, just announce to screen reader
            AccessibilityManager.announce("Goal completed!");
            return;
        }
     ...
     * Requirements: 3.1, 3.2 - Milestone celebrations with animations
     */
    async celebrateStreakMilestone(days: number) {
        // Strong haptic feedback for milestones
        HapticFeedback.achievement();

        // Play milestone sound if enabled
        if (this.soundEnabled) {
            await soundManager.playMilestoneSound();
        }

        const confettiConfig = getOptimizedConfettiConfig("streakMilestone");

        if (!confettiConfig) {
            AccessibilityManager.announce(`${days} day streak milestone reached!`);
            return;
        }

        // Trigger enhanced confetti for milestones
        setTimeout(() => {
            if (this.confettiRef?.current) {
                this.confettiRef.current.start();
            }
        ...
        // Additional confetti burst for major milestones
        if (days >= 100) {
            setTimeout(() => {
                if (this.confettiRef?.current) {
                    this.confettiRef.current.start();
    ...
    /**
     * Trigger daily progress completion
     */
    async celebrateDailyComplete() {
        HapticFeedback.success();

        // Light sound for daily completion
        if (this.soundEnabled) {
            await soundManager.playSuccessSound();
        }

        const confettiConfig = getOptimizedConfettiConfig("dailyComplete");

        if (!confettiConfig) {
            AccessibilityManager.announce("Daily progress completed!");
            return;
        }
...
// Global celebration manager instance
